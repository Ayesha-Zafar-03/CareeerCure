...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const register = async (email: string, full_name: string, password: string) => {
    // Registration endpoint only returns OTP, not access token
    // The access token is returned after email verification
    const res = await authApi.register({ email, full_name, password });
    return res.data; // Return the registration response with OTP info
  };
  ...
  return (
    <AuthContext.Provider value={{ user, token, login, verifyAdminLogin, register, logout, setUser, setToken, isLoading }}>
      {children}
