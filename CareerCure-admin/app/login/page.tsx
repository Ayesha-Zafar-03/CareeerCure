...
export default function LoginPage() {
  ...
  const handleSocialLogin = async (provider: 'google' | 'linkedin') => {
    ...
    try {
      ...
      if (!isConfigured) {
        alert(`${provider === 'google' ? 'Google' : 'LinkedIn'} OAuth is not configured yet. Please use email login.`);
        return;
      }
      ...
      setError(`${provider} login failed. Please try email login.`);
    } finally {
      ...
      <div className="w-full max-w-md">
        <div className="bg-surface rounded-2xl shadow-xl p-8 border border-line animate-fade-in-up">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-primary-dark mb-2">Admin Portal</h1>
            <p className="text-ink/60">Sign in to CareerCure Admin Panel</p>
          </div>
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-ink/60">
              Admin access only.{" "}
              <Link href="http://localhost:3000" className="text-primary hover:text-primary-dark font-medium">
                Go to User Portal
              </Link>
            </p>
          </div>
