  ...
  return (
    <ProtectedRoute>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 flex flex-col" style={{ height: "calc(100vh - 64px)" }}>
        <h1 className="text-2xl font-bold text-primary-dark mb-1">AI Career Counselor</h1>
        <p className="text-ink/50 text-sm mb-4">Powered by Groq LLaMA3 + RAG knowledge base</p>
          ...
          {messages.map((msg, i) => (
            <div key={i} className={clsx("flex gap-3", msg.role === "user" ? "flex-row-reverse" : "flex-row")}>
              <div className={clsx(
                ...
                msg.role === "assistant" ? "bg-primary/10 text-primary" : "bg-primary/10 text-ink/60"
              )}>
            ...
            <div className="flex gap-3">
              ...
              <div className="bg-surface border border-line px-4 py-3 rounded-2xl rounded-tl-sm">
                <div className="flex gap-1">
                  <span className="w-2 h-2 bg-line rounded-full animate-bounce" style={{ animationDelay: "0ms" }}></span>
                  <span className="w-2 h-2 bg-line rounded-full animate-bounce" style={{ animationDelay: "150ms" }}></span>
                  <span className="w-2 h-2 bg-line rounded-full animate-bounce" style={{ animationDelay: "300ms" }}></span>
                </div>
              </div>
            </div>
          )}
        ...
        {messages.length === 1 && (
          <div className="flex flex-wrap gap-2 mb-3">
            {SUGGESTIONS.map((s) => (
              <button key={s} onClick={() => sendMessage(s)} className="text-xs bg-primary/10 hover:bg-primary/20 text-ink px-3 py-1.5 rounded-full transition-colors">
                {s}
              </button>
            ))}
          </div>
